...
- Fix: platform skill installation now falls back to the packaged generic skill when a platform-specific skill template is absent, so advertised install commands no longer fail on incomplete wheels.
- Docs: added `ARCHITECTURE.md` and clarified the difference between global slash-command installation and repo-local always-on configuration.
- Cleanup: removed unused imports, empty f-strings, and stale local variables; the Python package now passes `ruff check`.

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `dummyindex install` when multiple platforms were previously installed — `dummyindex install` now refreshes `.dummyindex_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/dummyindex-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `dummyindex cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/dummyindex --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.dummyindex_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `dummyindex update .` and `dummyindex cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `dummyindex-out/dummyindex-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when dummyindex is run from a subdirectory (#410)
- Fix: `dummyindex hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `dummyindex-out/`, `.dummyindexignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: dummyindex watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `dummyindex vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `dummyindex watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `dummyindex-out/wiki/` (#229, #354)
- Fix: `dummyindex install --platform opencode` now also installs the `.opencode/plugins/dummyindex.js` plugin, matching what `dummyindex opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/dummyindex-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `dummyindex update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `dummyindex update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `dummyindex kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `dummyindex update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the dummyindex binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `dummyindex query` and `dummyindex explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `dummyindex kiro install` writes `.kiro/skills/dummyindex/SKILL.md` (invoked via `/dummyindex`) and `.kiro/steering/dummyindex.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `dummyindex query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `dummyindex query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `dummyindex-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.dummyindexignore` patterns from a parent directory now fire correctly when dummyindex is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.dummyindexignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `dummyindex install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `dummyindex hermes install` writes skill to `~/.hermes/skills/dummyindex/SKILL.md` and AGENTS.md (#251)
...
- Fix: `dummyindex claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `dummyindex`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `dummyindex antigravity install` writes `.agent/rules/dummyindex.md` (always-on rules) and `.agent/workflows/dummyindex.md` (`/dummyindex` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has dummyindex installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.dummyindexignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and dummyindex transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/dummyindex add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `dummyindex-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'dummyindex[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and dummyindex transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/dummyindex add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `dummyindex-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'dummyindex[video]'` for faster-whisper + yt-dlp
...
- Fix: dummyindex install --platform gemini now also copies the skill file to ~/.gemini/skills/dummyindex/SKILL.md so the /dummyindex trigger works in Gemini CLI (#174)
...
- Fix: `dummyindex install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `dummyindex install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_dummyindexignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `dummyindex install --platform aider` copies skill to `~/.aider/dummyindex/SKILL.md`; `dummyindex aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `dummyindex install --platform copilot` copies skill to `~/.copilot/skills/dummyindex/SKILL.md`; `dummyindex copilot install/uninstall` for skill management (#134)
...
- Fix: `dummyindex codex install` (and opencode) no longer exits early when `AGENTS.md` already has the dummyindex section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `dummyindex gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `dummyindex cursor install` writes `.cursor/rules/dummyindex.mdc` with `alwaysApply: true` so the graph context is always included; `dummyindex cursor uninstall` removes it (#137)
...
- Fix: README clarifies `dummyindex` is the only official PyPI package — other `dummyindex*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `dummyindex opencode install` now writes `.opencode/plugins/dummyindex.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.dummyindexignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `dummyindex save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `dummyindex save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p dummyindex-out` now runs before writing `.dummyindex_python` in `skill.md` — prevents write failure on first run; `.dummyindex_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `dummyindex query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.dummyindex_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`dummyindex install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from dummyindex binary)
...
- Fix: `dummyindex codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.dummyindex_*.json`) now live in `dummyindex-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `dummyindex install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$dummyindex .` not `/dummyindex .` (#36)
...
- Add: `dummyindex query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `dummyindex-out/obsidian`
...
- Add: `dummyindex install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `dummyindex codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `dummyindex claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `dummyindex claude install` — writes dummyindex section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `dummyindex claude uninstall` — removes section and hook
- Add: `dummyindex hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `dummyindex hook uninstall` / `hook status`
- Add: `dummyindex benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (dummyindex-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if dummyindex-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what dummyindex can do
...
- Add: `dummyindex/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install dummyindex` → `pip install dummyindex` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → dummyindex answer framing
...
- Memory feedback loop - Q&A results saved to `dummyindex-out/memory/`, extracted on `--update`
...
- `dummyindex install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
