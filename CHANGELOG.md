...
- The managed `CLAUDE.md` block (generated by `context/output/bootstrap.py`) no
  longer claims the index "auto-refreshes on commit/edit". It now states the
  index is updated explicitly — `rebuild --changed` for the deterministic
  backbone, `reconcile` to fold new/changed code into the curated taxonomy — and
  that the SessionStart hook only *reports* drift.
- Hook count corrected across the guide (`01-purpose`, `03-architecture`,
  `07-cli`, `09-lifecycle`), `COMMANDS.md`, and `reference/01-conventions.md`:
  the core install wires **three** Claude hooks (SessionStart drift report, Stop
  handoff nudge, PreCompact breadcrumb), none of which rebuild the index —
  earlier docs said "one hook (v0.13.5)".
- Removed the false "LLM-driven extraction for the rest / no language is
  unsupported" claim: extraction is tree-sitter + regex only; languages outside
  the supported set are skipped (LLM extraction remains roadmapped).
- `SECURITY.md`: the graph-viewer XSS mitigation now documents the real
  client-side `escapeHtml()` + D3 `.text()` path (the old `sanitize_label`/pyvis
  embed was removed in v0.6); stale module names (`pipeline.io.detect`) and the
  supported-versions table refreshed.
- The onboarding "auto-refresh hook" preference is renamed "session hooks" in
  the skill prose; the `DUMMYINDEX_AUTO_REFRESH` sentinel constant is retained
  for upgrade compatibility (documented inline).
- `council/10-structural-review.md`: dropped the anachronistic "v0.8" split
  label and scoped the merge note to `features-merge`'s actual remit
  (trivial-feature consolidation).
- Roadmap records the post-v0.15 commit-anchored reconcile + auto-handoff hooks.
