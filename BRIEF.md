...
**Date:** 2026-05-24
**Name:** **dummyIndex** (this is an evolution of the existing dummyindex/graphify project, not a new product)
**Repo:** This repo — `/mnt/windows-ssd/Projects/memory/graphify/`
...
## 2. Why this exists (vs graphify, Aider, etc.)

| Tool | What it gives the agent | What it doesn't |
|---|---|---|
| **graphify** (current) | One-shot knowledge graph as HTML/JSON for human exploration | Not designed for agent retrieval at task time; not a stable on-disk contract |
...
1. **Deterministic-first.** Anything derivable from AST/git/config is computed without LLM calls. LLM calls are reserved for summaries, naming, and convention inference. Borrowed wholesale from graphify's two-tier design.
...
4. **Confidence-tagged** (graphify-derived). Every edge and inference labeled `EXTRACTED` / `INFERRED` / `AMBIGUOUS`. The agent sees what's known vs guessed.
...
6. **Live, incremental** (graphify cache, SHA-256 keyed). Re-runs only re-process changed files. `--watch` mode rebuilds on save.
...
├── tree.html                   # Optional human viewer (reuses graphify's D3 viewer)
│
...
│
├── flows/                      # Hyperedges from entry → terminals (graphify's flow synth)
│   ├── index.md
│   ├── <flow-slug>.md          # e.g. "login-flow.md" — files + symbols + ordering
│   └── flows.json
│
├── features/                   # Product-level capabilities (graphify's feature synth)
│   ├── index.md
...
- A `.context/` writer (graphify writes `dummyindex-out/`; different shape, different audience)
...
- **Incremental re-runs** reuse graphify's SHA-256 cache; unchanged files cost zero.
...
- Reuse graphify's `detect` + `extract` directly to produce a flat `map/symbols.json`.
...
- Reuse graphify's flow + feature synthesis, emit `.context/flows/`, `.context/features/`.
...
**Resolved (2026-05-24):**
- ✅ Location: this repo (`/mnt/windows-ssd/Projects/memory/graphify/`).
...
1. **Languages, phase 1.** Start with Python + TypeScript only, or take graphify's 25-language reach from day one? My recommendation: Python + TS for v2.0 (where most agent-coded projects live), then expand.
...
- [safishamsi/graphify](https://github.com/safishamsi/graphify) — upstream knowledge graph engine
