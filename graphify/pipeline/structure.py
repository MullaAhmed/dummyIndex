...
# .graphifyignore / .gitignore style patterns.
_STRUCTURE_IGNORE_FILES = (".codeindexignore", ".graphifyignore")
...
def build_structure(extraction: dict, code_files: list[Path], root: Path) -> dict:
    ...
    # Start with graphify's detected code files, then augment with HTML, ipynb,
...
def _discover_extra_source_files(root_abs: Path, code_files: list[Path]) -> list[Path]:
    ...
    Files and directories are excluded when they:
      * match a pattern in ``.codeindexignore`` (preferred) or ``.graphifyignore``
...
class _StructureIgnoreMatcher:
    ...
    ``.graphifyignore`` file at ``root``. Supports the common 95% of gitignore
