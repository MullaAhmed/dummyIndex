...
def build_structure(extraction: dict, code_files: list[Path], root: Path) -> dict:
    ...
    source_nodes = [n for n in extraction.get("nodes", []) if isinstance(n, dict) and "id" in n]
    source_edges = [e for e in extraction.get("edges", []) if isinstance(e, dict)]
    ...
    _ensure_files_for_all_paths(code_files, root_abs, nodes, file_ids_by_rel)

    root_id, root_label = _add_folders(code_files, root_abs, nodes, file_ids_by_rel, hierarchy_edges)
...
def _rel_path(source_file: str, root_abs: Path) -> str:
    """Return a forward-slash path relative to root_abs, or ``""`` if not under it."""
    if not source_file:
        return ""
