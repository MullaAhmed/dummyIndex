"""Tests for dummyindex install --platform routing."""
...
PLATFORMS = {
    "claude": (".claude/skills/dummyindex/SKILL.md",),
    "codex": (".agents/skills/dummyindex/SKILL.md",),
    "opencode": (".config/opencode/skills/dummyindex/SKILL.md",),
    "claw": (".openclaw/skills/dummyindex/SKILL.md",),
    "droid": (".factory/skills/dummyindex/SKILL.md",),
    "trae": (".trae/skills/dummyindex/SKILL.md",),
    "trae-cn": (".trae-cn/skills/dummyindex/SKILL.md",),
    "windows": (".claude/skills/dummyindex/SKILL.md",),
}


def _install(tmp_path, platform):
    from dummyindex.__main__ import install
    with patch("dummyindex.__main__.Path.home", return_value=tmp_path):
        install(platform=platform)


def test_install_default_claude(tmp_path):
    _install(tmp_path, "claude")
    assert (tmp_path / ".claude" / "skills" / "dummyindex" / "SKILL.md").exists()
...
def test_install_opencode(tmp_path):
    ...
    assert (tmp_path / ".config" / "opencode" / "skills" / "dummyindex" / "SKILL.md").exists()


def test_install_claw(tmp_path):
    _install(tmp_path, "claw")
    assert (tmp_path / ".openclaw" / "skills" / "dummyindex" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "dummyindex" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "dummyindex" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "dummyindex" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    import dummyindex
    skill = (Path(dummyindex.__file__).parent / "skills" / "skill-codex.md").read_text()
...
def test_opencode_skill_contains_mention():
    ...
    import dummyindex
    skill = (Path(dummyindex.__file__).parent / "skills" / "skill-opencode.md").read_text()
...
def test_claw_skill_is_sequential():
    ...
    import dummyindex
    skill = (Path(dummyindex.__file__).parent / "skills" / "skill-claw.md").read_text()
...
def test_all_skill_files_exist_in_package():
    ...
    import dummyindex
    pkg = Path(dummyindex.__file__).parent / "skills"
...
def _agents_install(tmp_path, platform):
    from dummyindex.__main__ import _agents_install as _install_fn
    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from dummyindex.__main__ import _agents_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "dummyindex" in agents_md.read_text()
...
def test_agents_install_idempotent(tmp_path):
    ...
    assert content.count("## dummyindex") == 1
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained dummyindex section
...
def test_agents_uninstall_preserves_other_content(tmp_path):
    ...
    content = agents_md.read_text()
    assert "Do not break things." in content
    assert "## dummyindex" not in content
...
def test_opencode_agents_install_writes_plugin(tmp_path):
    """opencode install writes .opencode/plugins/dummyindex.js."""
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "dummyindex.js"
...
def test_opencode_agents_install_registers_plugin_in_config(tmp_path):
    ...
    assert any("dummyindex.js" in p for p in config.get("plugin", []))
...
def test_opencode_agents_install_merges_existing_config(tmp_path):
    ...
    assert any("dummyindex.js" in p for p in config["plugin"])
...
def test_opencode_agents_uninstall_removes_plugin(tmp_path):
    ...
    if config_file.exists():
        config = _json.loads(config_file.read_text())
        assert not any("dummyindex.js" in p for p in config.get("plugin", []))


# ── Cursor ────────────────────────────────────────────────────────────────────

def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/dummyindex.mdc."""
    from dummyindex.__main__ import _cursor_install
    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "dummyindex.mdc"
    assert rule.exists()
    content = rule.read_text()
    assert "alwaysApply: true" in content
    assert "dummyindex-out/GRAPH_REPORT.md" in content
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from dummyindex.__main__ import _cursor_install, _cursor_uninstall
    _cursor_install(tmp_path)
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from dummyindex.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from dummyindex.__main__ import gemini_install
    gemini_install(tmp_path)
    md = tmp_path / "GEMINI.md"
    assert md.exists()
    assert "dummyindex-out/GRAPH_REPORT.md" in md.read_text()
...
def test_gemini_install_writes_hook(tmp_path):
    ...
    assert any("dummyindex" in str(h) for h in hooks)
...
def test_gemini_install_idempotent(tmp_path):
    ...
    gemini_install(tmp_path)
    md = tmp_path / "GEMINI.md"
    assert md.read_text().count("## dummyindex") == 1
...
def test_gemini_uninstall_removes_section(tmp_path):
    from dummyindex.__main__ import gemini_install, gemini_uninstall
    gemini_install(tmp_path)
...
def test_gemini_uninstall_removes_hook(tmp_path):
    ...
    if settings_path.exists():
        ...
        assert not any("dummyindex" in str(h) for h in hooks)

def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from dummyindex.__main__ import gemini_uninstall
