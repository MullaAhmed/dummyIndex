...
def _skill_src(name: str) -> Path:
    exact = _SKILLS_DIR / name
    if exact.exists():
        return exact
    generic = _SKILLS_DIR / "skill.md"
    if name != "skill.md" and generic.exists():
        return generic
    return exact
    ...
    "hooks": [
        {
            ...
            "command": (
                ...
                r"""echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"dummyindex: Structure graph exists. Navigate dummyindex-out/structure_graph.json first to locate the relevant folder/file/class/function. If dummyindex-out/feature_graph.json exists, check which named features (capabilities like Auth, Payments, Reporting) the target belongs to. If dummyindex-out/flow_graph.json exists, also consult it for end-to-end flows. Then consult dummyindex-out/GRAPH_REPORT.md for community/architectural context before searching raw files."}}' """
                "|| [ -f dummyindex-out/graph.json ] && "
                r"""echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","additionalContext":"dummyindex: Knowledge graph exists. Read dummyindex-out/GRAPH_REPORT.md for god nodes, named features, community structure, and flows before searching raw files."}}' """
                "|| true"
            ),
        }
    ],
...
def _kiro_install(project_dir: Path) -> None:
    ...
    if steering_dst.exists() and _KIRO_STEERING_MARKER in steering_dst.read_text(encoding="utf-8"):
        print("  .kiro/steering/dummyindex.md  ->  already configured")
    else:
        steering_dst.write_text(_KIRO_STEERING, encoding="utf-8")
        print("  .kiro/steering/dummyindex.md  ->  always-on steering written")
...
def _agents_install(project_dir: Path, platform: str) -> None:
    ...
    if target.exists():
        ...
        if _AGENTS_MD_MARKER in content:
            print("dummyindex already configured in AGENTS.md")
        else:
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print("  hermes install          write dummyindex section to AGENTS.md (Hermes)")
        print("  hermes uninstall        remove dummyindex section from AGENTS.md")
    ...
    if cmd == "install":
        ...
        from dummyindex.runtime.watch import _build_structure_artifacts, _build_flow_artifacts, _build_feature_artifacts
        code_files = [
            Path(n["source_file"])
        ...
        _build_feature_artifacts(G, communities, gods, labels, out)
        print(f"Done — {len(communities)} communities. GRAPH_REPORT.md, graph.json, graph.html, structure_graph, flow_graph and feature_graph updated.")
