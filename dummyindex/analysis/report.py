...
def generate(
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if isolated:
            ...
            lines.append(f"- **{len(isolated)} isolated node(s):** {', '.join(f'`{label}`' for label in isolated_labels)}{suffix}")
        ...
        if thin_communities:
            for cid, nodes in thin_communities.items():
                ...
                lines.append(f"- **Thin community `{label}`** ({len(nodes)} nodes): {', '.join(f'`{node_label}`' for node_label in node_labels)}")
