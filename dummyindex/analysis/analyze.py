...
def suggest_questions(
    ...
    if G.number_of_edges() > 0:
        ...
        for node_id, score in bridges:
            ...
            if neighbor_comms:
                ...
                questions.append({
                    "type": "bridge_node",
                    "question": f"Why does `{label}` connect `{comm_label}` to {', '.join(f'`{label_text}`' for label_text in other_labels)}?",
    ...
    if isolated:
        ...
        questions.append({
            "type": "isolated_nodes",
            "question": f"What connects {', '.join(f'`{label_text}`' for label_text in labels)} to the rest of the system?",
