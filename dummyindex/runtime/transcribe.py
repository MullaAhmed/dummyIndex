...
_DEFAULT_MODEL = "base"
_TRANSCRIPTS_DIR = "dummyindex-out/transcripts"
_FALLBACK_PROMPT = "Use proper punctuation and paragraph breaks."


def _model_name() -> str:
    return os.environ.get("DUMMYINDEX_WHISPER_MODEL", _DEFAULT_MODEL)
...
def _get_whisper():
    try:
        ...
        raise ImportError(
            ...
            "Run: pip install 'dummyindex[video]'"
        ) from exc
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via DUMMYINDEX_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("DUMMYINDEX_WHISPER_PROMPT")
    if override:
        return override
