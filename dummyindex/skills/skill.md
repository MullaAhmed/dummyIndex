...
- **`/dummyindex-build`** — drives the proposal's `checklist.md` to completion (`dummyindex context build`), dispatching via each task's mapped `subagent_type` (or `general-purpose` fallback), **verify-before-tick**, then a post-build learning step (success / error→working-path / user correction → `equip patch`), then `rebuild --changed` to re-index and close the loop.
