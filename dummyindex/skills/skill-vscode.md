...
Synthesize **named capability-level features** by combining communities + flows + god nodes + folders + docs. Each feature is a hyperedge — shared utilities (logger, db, validators) belong to every feature that uses them. Stage A is deterministic; Stage B names features via subagents using main-graph community labels + god nodes as advisory context. Skip if no communities exist.
...
Then delete `dummyindex-out/.dummyindex_feature_names_todo.json`, `.dummyindex_feature_names_results.json`, `.dummyindex_feature_names_*.json`, `.dummyindex_features_pending.json`. The cache file `.dummyindex_feature_names.json` and the user override file `features.yaml` are preserved.
