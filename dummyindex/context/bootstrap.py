...
This repo has a generated, always-current context index at `.context/`. **Read `.context/HOW_TO_USE.md` before any non-trivial task** — it answers most "where / how / what" questions without grepping. The index auto-refreshes on commit/edit; trust it. When the index disagrees with the code, the code wins — flag it and run `dummyindex context rebuild --changed`."""
