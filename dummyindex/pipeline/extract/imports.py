"""Per-language import-statement handlers.

Each `_import_<lang>` reads an import node, computes the target node ID,
and appends an edge to the caller-supplied `edges` list. `_extract_generic`
dispatches to the right handler via `LanguageConfig.import_handler`.
"""
