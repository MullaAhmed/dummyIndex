...
- **Docstring attachment** — a symbol's `rationale_for` neighbor's text is attached to its subject line and rows, whitespace-flattened and clipped to 120 chars; when several rationale nodes attach, the lowest rationale node id wins deterministically (`load.py:98-113`, `verbs.py:41-47`, `constants.py:12`).
