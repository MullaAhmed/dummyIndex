...
- `context/enums.py` also carries the codebase-scan alphabets, which share `DocConfidence`'s `__str__ = str.__str__` pin for the same reason: `ScanNodeKind` (`entry cron agent model tool service store external`) and `ScanEdgeKind` (`calls reads writes triggers`). They are closed sets so the scan viewer can map a kind to a column, a colour, and a glyph without special-casing — and so an unknown kind is a validation error rather than an unstyled box.
