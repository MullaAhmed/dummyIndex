...
- Doc-hygiene guards assert the generated prose carries the binding `— via` gate, read-only reconcile wording, the `feature_id`/INDEX.json field contract, and never the known-bad `install --scope user` remedy or phantom `dummyindex --recouncil` CLI verb (`test_skills_doc_hygiene.py:135-224`, `:535-576`). They also pin the codebase-scan council doc's ranked-seed contract — edit-the-ranked-shortlist wording, `symbolRef`/`evidence` requirements grounded in `graph-communities.json`, the 120-node/240-edge caps with the 40–80 aim, and `skill.md`'s matching scan-phase summary (`test_skills_doc_hygiene.py:579-641`).
