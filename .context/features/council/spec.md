...
The council is dummyindex's per-feature documentation pipeline: for every non-trivial feature it drives a fixed sequence of LLM stages — dev drafts `spec.md`, an architect reorganises `plan.md`, critics file `concerns.md`, dev narrates flows, and a tree-enrich pass (on by default; opt out per run with `--no-tree-enrich`) — fanning *independent* features out to parallel Task subagents. The code in this feature is the **deterministic plumbing** the LLM skill calls between dispatches; it runs no LLM. One stage deliberately sits outside this machinery: `council/58-codebase-scan.md` (skill phase 4.8) authors the repo-wide codebase scan at `features/graph.json` — an **edit of the PageRank-ranked deterministic seed** (keep / rework / drop over a ranked shortlist whose nodes carry `evidence` + `symbolRef`, groups grounded in `features/graph-communities.json`; caps 120 nodes / 240 edges, aim 40–80), never an invent-from-scratch rewrite (`dummyindex/skills/council/58-codebase-scan.md:11-33`). It is **whole-repo and single-agent**, not per-feature, so it is not a `CouncilStage`, never appears in `active_stages`, and has no `_council-log.json` entry — its completion marker is the artifact's own `confidence: INFERRED`, and its feedback loop is `dummyindex context scan-check` rather than a critic. From the append-only per-feature audit logs alone it computes the *earliest incomplete stage* across all features and the concrete dispatch units (which agent, for which feature, in which role) to launch for it, then records each invocation's outcome so a crashed or partial run resumes without re-doing finished work.
...
- `--no-tree-enrich` — drops the stage-5 tree pass. The pass is **on by default** in every depth (`cli/council_batch.py:104-108`); `--tree-enrich` is retained as an accepted no-op for older callers. The domain still takes `tree_enrich` as a required keyword — the default lives at the CLI boundary only.
