...
- `_resolve_python_dispatch(per_file: list[dict], paths: list[Path]) -> list[dict]` — returns new `calls` (enum-keyed dict values) / `imports_from` (in-function `from X import Y`) edges, all INFERRED (score 0.8); returns `[]` when `tree_sitter_python` is unavailable; skips a file whose cached result is keyed to another path shape (`file_nid not in file_node_ids`) and any unparseable file — `pipeline/extract/python_dispatch.py:295-344`. The caller (`extract`) runs it **before** the `id_remap` pass so file-node-sourced edges are remapped with everything else, dedups its edges against already-extracted ones by `(source, target, relation)`, and degrades any failure to a logged warning — `pipeline/extract/__init__.py:211-235`
- `_build_symbol_index(per_file) -> (index, ambiguous)` — `(module stem, symbol name) → node id` across all Python files; a key claimed by more than one source file (or node id) lands in `ambiguous` and is never resolved — `pipeline/extract/python_dispatch.py:73-97`
- `_collect_bindings(root, source) -> (bound_names, from_stems)` — names bound by imports anywhere in the file, plus `name → module stem` for plain `from X import name`; aliased from-imports bind the alias but resolve no stem, and a name from-imported from two different modules is dropped as ambiguous — `pipeline/extract/python_dispatch.py:113-171`
- `_walk_file(scan, node, scope_nid, fn_depth)` — recursive AST walk mirroring `generic.py`'s node-id scheme; `scope_nid` is the nearest enclosing scope that has an extracted node — `pipeline/extract/python_dispatch.py:247-292`
