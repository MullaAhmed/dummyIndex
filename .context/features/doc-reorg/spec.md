...
- **backup** — **re-asserts the clean tree itself** (`require_clean_tree`) and exits `1` on a dirty or unknown-git tree before snapshotting anything; then snapshots every in-scope doc under `.context/_doc_backups/<utc>/`, writes a `manifest.json`, prints the count, destination, and the exact restore command (or the backup dict under `--json`). A `BackupError` also exits `1` (`cli/doc_reorg.py:77-105`).
...
The reorg **phase** is no longer opt-in — it runs as skill Phase 5.5 of the normal pipeline (`skills/council/60-doc-reorg.md:1-9`), and `/dummyindex --reorg-docs` only runs that phase *alone*. That is why `backup` now enforces the gate rather than trusting the operator to have run `guard` first: with the phase always in the pipeline, a snapshot taken on a dirty tree would record already-modified docs as "originals" and a later `restore` would faithfully reproduce that dirty state instead of the committed baseline. `restore` is deliberately **not** gated — restoring is the recovery path.
