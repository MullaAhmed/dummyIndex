...
- `Config.from_dict(payload) -> Config` / `to_dict()` (`config.py:176`); `read_config(context_dir) -> Optional[Config]` (`config.py:361`); `write_config(context_dir, config) -> Path` — atomic; stamps `dummyindex_version` on every write (`config.py:420`); `migrate_config_in_place(context_dir) -> bool` — re-persists a loadable-but-stale config (schema bump or renamed value), preserving choices; `False`/no-op on absent, current, or unreadable configs (`config.py:393`, guard `_needs_migration` at `config.py:378`); `reconcile_wired_with_equipment(context_dir) -> bool` — unions `kind=plugin` items from `equipment.json` (keyed on `<plugin>@<marketplace>`) into `config.wired`, healing plugins a v1→v2 migration would drop; idempotent, `False`/no-op on absent config, absent/unreadable `equipment.json`, or nothing-to-add (`config.py` `reconcile_wired_with_equipment`); `default_config() -> Config` — seeds `wired` from `DEFAULT_PLUGINS` (`config.py:321`); `resolve_depth(context_dir, command, depth_flag) -> CouncilMode` — the single per-command depth seam every command resolves through (precedence: flag → `command_depths[command]` → `mode` → `STANDARD`) (`config.py:339`); `current_dummyindex_version() -> str` (`config.py:115`).
