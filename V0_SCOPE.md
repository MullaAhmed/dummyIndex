...
Running `dummyindex init` on this repo (graphify) produces:
...
1. `dummyindex init` runs clean on this repo (graphify) and produces every file listed in §4.
...
7. Conventions inference produces sensible output on graphify (manual spot-check: are the rules accurate?) and on the sample fixture (assertion-driven).
...
**Acceptance:** Golden-file match on the sample fixture. On graphify itself, the tree has every top-level module as L1, every file as L2, every class/top-level function as L3, methods as L4.
...
**Acceptance:** Running on graphify produces a markdown file whose rules can be manually verified against the code. Test on the sample fixture asserts specific expected rules.
...
`evals/v0/tasks.yaml` — 5–10 representative coding tasks on the graphify repo and the sample fixture. Examples:
...
- The graphify maintainer (whoever reviews PR 6) is the human judge for quality and task success — not the author of v0.
...
| Sample fixture is too small to detect noise | Mitigate by also running on graphify itself + at least one external fixture (we'll pick something open-source, ~5k LOC). |
| `pipeline.structure` output shape doesn't quite fit PageIndex tree | Reshape in `tree.py`; if structural changes to graphify's pipeline are needed, they go in their own PR before PR 3. |
...
1. **Languages, v0.** I'd start with Python + TypeScript only — covers ~80% of agent-coded projects and limits the surface area of the chunking heuristic. graphify's full 25-language support stays available; v0 just doesn't *test* against the others. Confirm or override.
