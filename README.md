...
> **Work in progress:** dummyindex is still evolving. It is built on top of DummyIndex's [https://github.com/safishamsi/dummyindex] knowledge-graph approach and extends it with agent-first workflows, richer code extraction, multimodal corpus support, persistent graph querying, assistant installation hooks, and local update tooling. The goal is to make DummyIndex's structural map useful during everyday AI-assisted development: not just to visualize a project once, but to help agents answer architecture questions, trace implementation paths, preserve context across sessions, and explain why parts of a codebase are connected.
...
├── flow_graph.html / flow_graph.json      end-to-end flows derived from call paths and entry points
├── feature_graph.html / feature_graph.json  feature-level map across structure, flows, and docs
├── wiki/            optional agent-crawlable markdown wiki
├── run_log.json     run summary and artifact statistics
...
- `dummyindex install --platform <name>` installs the global slash-command skill for that assistant.
- `dummyindex <platform> install` writes repo-local always-on context such as `AGENTS.md`, `CLAUDE.md`, hooks, or rules files.

The package ships a generic skill plus a Codex-specific skill. If a platform-specific skill template is not packaged, the installer falls back to the generic skill instead of failing.
...
**Aider, OpenClaw, Factory Droid, Trae, and Hermes** write the same rules to `AGENTS.md` in your project root. These platforms don't support tool hooks, so AGENTS.md is the always-on mechanism. Install the global slash-command skill separately with `dummyindex install --platform <platform>` when needed.
...
> ```
> Also note: the official PyPI package is `dummyindex`; similarly named packages are not affiliated with this project.
...
dummyindex hermes install             # AGENTS.md (Hermes)
...
| Docs | `.md .mdx .html .txt .rst` | Concepts + relationships + design rationale via the assistant model |
| Office | `.docx .xlsx` | Converted to markdown then extracted via the assistant model (requires `pip install dummyindex[office]`) |
| Papers | `.pdf` | Citation mining + concept extraction |
| Images | `.png .jpg .jpeg .webp .gif .svg` | Vision-capable assistant extraction - screenshots, diagrams, any language |
| Video / Audio | `.mp4 .mov .mkv .webm .avi .m4v .mp3 .wav .m4a .ogg` | Transcribed locally with faster-whisper, transcript fed into semantic extraction (requires `pip install dummyindex[video]`) |
...
NetworkX + Leiden (graspologic) + tree-sitter + vis.js. Semantic extraction runs through whichever model your assistant platform provides. Video transcription uses faster-whisper + yt-dlp (optional, `pip install dummyindex[video]`). No Neo4j required, no server, runs entirely locally.
