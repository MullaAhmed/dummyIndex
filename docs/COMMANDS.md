...
| `dummyindex context statusline [path] [--root DIR]` | Print the cached `.context/` freshness badge (`[ctx ✓]` / `[ctx: E edited · A anchored]`) for a shell `statusLine`. Reads the pre-computed badge cache (written by the SessionStart `plan-update` path); never recomputes. Missing/malformed cache or any error → empty stdout, exit 0. |
| `dummyindex context drift-ack [path] [--root DIR] (--feature ID [--path REL] [--reason TEXT] \| --list \| --clear)` | Acknowledge drift rows as false positives: append-only entries in gitignored `cache/drift-acks.json`, suppressed only while the file's bytes match `acked_sha` (any edit auto-expires). `--list` prints acks; `--clear` drops them. Off-git repos use content sha256. |
...
| `dummyindex context plan-update [path] [--root DIR] [--json]` | Drift report for the SessionStart hook (advisory; markdown to stdout). `--json` swaps it for a stable envelope: `{edited, anchored: {unassigned_new_files, awaiting_enrichment, drifted_features}, suppressed, acked}`. |
...
| `dummyindex context reconcile-stamp [path] [--root DIR] [--force] [--heal-orphaned]` | **Write boundary** — advance the anchor to HEAD once everything's reconciled. Refuses (exit 1) while unassigned files / awaiting-enrichment features remain (not on drift alone); `--force` overrides + warns; `--heal-orphaned` opts into re-baselining an orphaned (history-rewritten) anchor at HEAD with a loud warning instead of refusing. Off-git is a no-op. |
| `dummyindex context maintain plan\|begin\|next\|done\|stamp\|status [--max-features N] [--all] [--run NAME] [--json] [--root DIR]` | One-command maintenance loop over committed `.context/fleet/maintain-<ts>/` run state. `plan` prints the ordered work list with `estimate:` lines; `begin` writes RUN.md + state.json (requires `--max-features N` or `--all`); `next` prints the earliest incomplete unit (council-batch frontier semantics, resume-safe); `done --feature ID [--stage N]` ticks a unit; `stamp --feature ID` wraps `reconcile-stamp` and ticks on success; `status` reports counts/elapsed/heuristic remaining. No LLM calls — the host skill dispatches one subagent per unit. |
