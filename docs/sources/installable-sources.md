...
| `anthropics/skills` | Anthropic's public Agent Skills repo (document skills, artifacts, etc.) | 149,936 | 2026-06-09 | trusted | yes — **but seeded as `is_collection=True`; repo now ships `marketplace.json` (drift, see note)** |
| `anthropics/claude-code` | Claude Code main repo; ships first-party plugins (plugin-dev, hookify, code-review, ...) | 132,068 | 2026-06-12 | trusted | no |
...
| `obra/superpowers` | Agentic skills framework + development methodology (TDD, debugging, planning skills) | 225,964 | 2026-06-12 | untrusted | no |
| `affaan-m/ECC` | Agent harness optimization system: skills, instincts, memory, security | 214,285 | 2026-06-11 | untrusted | yes |
| `addyosmani/agent-skills` | Production-grade engineering skills for AI coding agents | 56,729 | 2026-06-11 | untrusted | no |
...
| `wshobson/agents` | Multi-harness plugin marketplace (Claude Code, Codex, Cursor, OpenCode, ...) | 36,668 | 2026-06-12 | untrusted | no |
| `kepano/obsidian-skills` | Obsidian skills: CLI, Markdown, Bases, JSON Canvas | 35,438 | 2026-06-08 | untrusted | no |
...
| `trailofbits/skills` | Security research, vulnerability detection, and audit-workflow skills | 5,680 | 2026-06-11 | untrusted | no |
| `obra/superpowers-marketplace` | Curated companion marketplace to superpowers | 1,067 | 2026-06-12 | untrusted | no |
...
**Drift note — `anthropics/skills`:** the seed entry marks it `is_collection=True`
(vendor, don't natively enable), but the repo now contains
`.claude-plugin/marketplace.json` and can be treated as a native marketplace. The
seed flag is stale; flag for review when SEED_MARKETPLACES is next touched.

**Strongest promotion candidates** (high stars, active, verified native):
`obra/superpowers`, `addyosmani/agent-skills`, `wshobson/agents`,
`anthropics/claude-code` (trusted), `trailofbits/skills` (security niche),
...
|---|---|---|---|---|---|---|
| `vercel-labs/agent-skills` | Vercel's official agent skills collection; canonical skills.sh source | 27,853 | 2026-06-10 | trusted (Vercel official) | skills.sh | no |
...
| Repo | What it offers | Stars | Last push | Trust | In SEED_MARKETPLACES |
|---|---|---|---|---|---|
| `msitarzewski/agency-agents` | Full "AI agency" of persona agents across 20+ domains (engineering, design, finance, ...) | 112,376 | 2026-06-07 | untrusted | yes — **but seeded WITHOUT `is_collection=True`; repo has no `marketplace.json` (drift, see note)** |
...
**Drift note — `msitarzewski/agency-agents`:** the seed entry treats it as a native
marketplace (no `is_collection` flag), but the repo has no `.claude-plugin/`
directory at all — it is category folders of agent `.md` files. The seed should
carry `is_collection=True`; flag for review when SEED_MARKETPLACES is next touched.
...
### Known seed drift (as of 2026-06-13)

- `anthropics/skills`: seeded `is_collection=True`, but now ships
  `marketplace.json` — could become a plain native marketplace seed.
- `msitarzewski/agency-agents`: seeded as native marketplace, but has no
  `marketplace.json` — should carry `is_collection=True`.

This document does not change code; both items need a deliberate code change +
