...
- **`equip add-specialist <capability>`** verb (+ `--specialist C` flag on `apply`) forces one explicitly; every template-backed specialist is generated by a plain `apply` regardless (no opt-in), and a re-apply carries forward every specialist already applied (reconstructed from the manifest), so it never drops one.
...
- **No `equipment.json` schema change** — stays v2. Existing four-core repos gain every template-backed specialist on their next apply (specialists are abilities, not opt-ins) and are stable across re-applies after that. The earlier deferral of *freeform* (non-template) generation stands — this is template-backed generation only.
